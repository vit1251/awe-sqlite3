...
static void awe_sqlite3_close_execute(napi_env env, void* data) {
...
static void awe_sqlite3_close_complete(napi_env env, napi_status status, void* data) {
    struct awe_sqlite3_close *arg = (struct awe_sqlite3_close *)data;
    napi_value ret;
...
napi_value Close(napi_env env, napi_callback_info info) {
    ...
    /* Step 2. Debug message */
#ifdef _DEBUG
    fprintf(stderr, "debug: Database %u close\n", wrapper->index);
#endif
    ...
    ptr->index = wrapper->index;
    ptr->db = wrapper->db; // Important!!!
    ptr->err = 0;
    //
    wrapper->db = NULL;
